...
class Raider:
    ...
    def headers_poll(self, token: str) -> dict:
        headers = Data().headers_poll
        headers.update({"Authorization": token})
        return headers
    
    def poll_spammer(self, token: str, channel_id: str, content: str):
        headers = self.headers_poll(token)
        ...
        while True:
            try:
                ...
                for i in range(10):
                    poll["poll"]["answers"].extend([
                        {"poll_media": {"text": content}}
                    ])
    ...
    def invite_spammer(self, token: str, guild_id: str):
        ...
        while True:
            try:
                ...
                match response.status_code:
                    case 200:
                        log("CREATED", C["green"], token[:25], f".gg/{response.json()['code']}")
                    case 429:
                        retry_after = response.json().get("retry_after")
                        sleep(retry_after)
                    case _:
    ...
    def token_checker(self, tokens: list, delay: float):
        valid = []
        ...
        src.menu.Menu().run(main, args, True, delay)
    ...
    def guild_checker(self, guild_id: str, tokens: list):
        ...
        args = [(token,) for token in tokens]
        src.menu.Menu().run(main_checker, args, True, 0.01)
