...
class Menu:
    ...
    def run(self, func, args, _sleep: bool=None, delay: float=None):
        threads = []
        Render().render_ascii()
        for arg in args:
            if _sleep:
    ...
    def joiner(self):
        ...
        if delay:
            self.run(self.raider.joiner, args, True, float(delay))
        else:
    ...
    def invite_spammer(self):
        ...
        if delay:
            self.run(self.raider.invite_spammer, args, True, float(delay))
        else:
    ...
    def poll_spammer(self):
        ...
        if delay:
            self.run(self.raider.poll_spammer, args, True, float(delay))
        else:
    ...
    def spammer(self):
        ...
        if "y" in massping:
            ...
            if not "y" in bypass:
                ...
                args = [(token, channel_id, message, guild_id, False, False, True) for token in self.tokens]
                self.run(self.raider.spammer, args)
        else:
            args = [(token, channel_id, message) for token in self.tokens]
            self.run(self.raider.spammer, args)
    ...
    def checker(self):
        ...
        if not delay: 
            delay = 0
        self.raider.token_checker(self.tokens, float(delay))
    ...
    def caller(self):
        ...
        self.run(self.raider.call_spammer, args, True, 0.03)
